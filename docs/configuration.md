...
- **Acquisition:** `ACQUISITION_BROWSER_ENABLED`,
  `ACQUISITION_MIN_MAIN_CONTENT_CHARS` (completeness floor, default 1500),
  `ACQUISITION_MIN_MAIN_CONTENT_CHARS_WITHOUT_STRUCTURE` (main text a page with no
  table and no PDF link needs to pass the floor, default 3000),
  `ACQUISITION_BASELINE_MAX_PDF_LINK_DROP`,
  `ACQUISITION_BASELINE_MAX_MAIN_CONTENT_DROP`,
  `ACQUISITION_BROWSER_NAVIGATION_TIMEOUT_SECONDS`,
  `ACQUISITION_BROWSER_SETTLE_MILLISECONDS`, `ACQUISITION_MAX_INTERACTIONS`,
  `ACQUISITION_MAX_LINKED_DOCUMENTS` (default 40), and
  `ACQUISITION_FRESHNESS_HOURS`.
